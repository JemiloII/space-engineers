...
const appendMethods = (path, actions = []) => {
  return entity => {
    ...
    if (actions.some(action => action === 'remove')) {
      entity.remove = () => {
        return send('DELETE', `${path}/${EntityId}`, {log: true});
      }
    }
...
const chat = async function ({count, date, message} = {}) {
  ...
  if (message) {
    return send('POST', path, {body: message, log: true});
  }
  ...
  const {Messages} = await send('GET', path, {qs: {Date: date, MessageCount: count}, log: true});
...
const players = async (search, methods = true) => {
